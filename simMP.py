...
def run_simulation(j, settings, QLNN, sim_steps=1000):
    ob = OrderBook()
    ...
    for i in range(1, sim_steps+1):
        ...
        # price process
        price += rng.normal(0, price_variance) #+ np.random.randint(-1, 2) * np.random.poisson(jump_rate)
        while price <= 10:
            price = 10 + rng.normal(0, price_variance) #+ np.random.randint(-1, 2) * np.random.poisson(jump_rate)

        for _ in range(5):
            # order price
            imbalance = ob.get_order_book_imbalance()

            pbid = 0.75 if imbalance < 0 else 0.25

            direction = rng.choice(["bid", "ask"], p=[pbid, 1-pbid])
            if direction =='bid':
                order_price = round(price - rng.integers(0, 5), 1)
            else:
                order_price = round(price + rng.integers(0, 5), 1)

            order_type = rng.choice(["limit", "market"], p=[0.9, .1])

            if order_type == "limit":
                quantity = rng.integers(1, 11)
                order =  Order(order_price, quantity, direction, order_type)
                ob.add_order(order)
        ...
        if (i % settings.time_step == 0) and (i > 200):
...
for sett in settings_list[:]:
    ...
    for j in tqdm(range(runs)):
        if j == 0:
            trader1, trader2 = run_simulation(j, sett, sim_steps=sim_steps, QLNN=None)
        else:
            trader1, trader2 = run_simulation(j, sett, sim_steps=sim_steps, QLNN=trader1)
